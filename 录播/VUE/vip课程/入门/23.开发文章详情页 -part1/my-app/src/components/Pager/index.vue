<template>
  <div class="pager-li" v-if="totalPage>1">
    <a
      ...
      >|&lt;&lt;</a
    >
    <a href="javascript:void(0);"  :class="{ disabled: current == 1 }" @click="changeCurrent(current-1)"
      >&lt;&lt;</a
    >
    <a href="javascript:void(0);"  v-for="(item, i) in pageArr" :key="i"  :class="{ active: current == item }" @click="changeCurrent(item)">{{
      item
    }}</a>
    <a href="javascript:void(0);"  @click="changeCurrent(current+1)" :class="{ disabled: current == totalPage}">&gt;&gt;</a>
    <a href="javascript:void(0);"  @click="changeCurrent(totalPage)"  :class="{ disabled: current == totalPage || totalPage< visiblePagesNumber}" v-if="totalPage>visiblePagesNumber">&gt;&gt;|</a>
  </div>
</template>
